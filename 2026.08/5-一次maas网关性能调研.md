...
看起来这是一个不到一千行代码的事件循环，但经验告诉我进入生产环境后这部分 C 代码将是需要重点维护的基础设施甚至有可能需要我们逐步开发成成熟网关，于是我打算先找有无现成工具思路和我的需求和数据路径一致，一个基于 HAProxy 的方案B如下：

![HAProxy 用户态 Filter 与异步后续处理组成的方案 B](assets/5-maas-performance/image-4.png)

方案B的这些操作是纯用户态的，舍弃了内核态处理。这里我们要注意一个点，方案A的主路虽然是内核态转发，但旁路仍然只能把数据转发到用户态做SSE解析处理，最终仍然会导致整个管线的速度卡在 用户态数据复制 + SSE解析，所以虽然方案A的内核态转发更快，但在高压的情况下不会持续很久就要被迫暂停等待旁路处理。