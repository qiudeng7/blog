...
明确这些边界之后，数据路径就可以重新设计了，借助 AI 设计了一套如下的数据流：
...
不过，这些“接线”并不是配置一次之后就会永久自动运行的内核流水线。用户态程序仍然需要通过 epoll 等机制等待 fd 就绪，然后针对每一批数据反复调用 `splice` 和 `tee`。所以更准确地说，主转发路径的 payload 停留在内核态，但整条路径的控制仍然在用户态。观察 Pipe 里的数据最终也需要被读取到用户态才能解析 SSE，这只是把解析工作从主转发数据路径中拆了出来，它是否会反过来拖慢主路径，仍然取决于观察 Pipe 能否被及时消费。

使用的时候需要注意 pipe 的容量是有限的。