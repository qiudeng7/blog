...
> 对于nas，我在 nas 上也用 linux，对于openwrt 和 mac ，我没有使用过，所以没有评价

## Termux

对于Termux，我曾经写过 [这篇文章](../2025/4-Termux.md)，引用其中一句话来介绍Termux：
...
4. ***wsl 在 2025年5月开源了***，[https://wsl.dev](https://wsl.dev) 感兴趣可以看看它的架构文档。
## Docker
...
1. [《UEFI 启动是如何工作的? 》](https://www.happyassassin.net/posts/2014/01/25/uefi-boot-how-does-that-actually-work-then/) 作者 Adam Williamson，写于2014年，文章用语介于通俗和专业之间，介绍了旧的BIOS的工作方式，UEFI是如何从问题出发来进行设计的，以及Boot Manager和Secure Boot。这篇文章写的非常好，我担心原链接丢失所以把这篇文章的原网页制作成了google机翻的 [中英双语版PDF](assets/1-操作系统和平台/uefi-boot-how-does-that-actually-work.pdf) ，看起来内容很多，但实际上这个网页有四分之三都是评论。
...
k8s的部署和使用是相对割裂的两个问题，有点像安装arch linux的各种方法，对新手来说有相对较高的门槛，也确实是一个影响重大、解决方案繁多的问题，五花八门的路径进一步加深了门槛，我觉得从 docker 迈入 k8s 的难度不亚于第一次从 windows 转向 linux。
...
对于生产环境的k8s，同样也可以使用k3s，或者rancher，rancher是一个持续监控和管理k8s集群本身（而不是k8s上的应用）的工具，也可以快速部署k8s。另外还有kubespray，这是ansible的脚本集合，ansible是一个python写的cli客户端工具，可以通过ssh的方式在远程服务器幂等的执行操作。也可以尝试一下我写的[kube-node-pool](https://github.com/qiudeng7/kube-node-pool),可以通过腾讯云的竞价式服务器组建集群并自动补充节点数量。


### 使用

使用，也就是在k8s上部署应用。最原始的操作是写yaml资源清单，然后用kubectl命令把yaml发送给api server，k8s就会创建并维护这些资源。

稍微高级一点的方式是kustomize和helm，如果把k8s的资源清单当做代码，则他们充当模块系统和包管理器的功能。kustomize可以简单解决资源之间的混乱，在kustomize的资源文件中可以加载其他yaml，并进行一定的覆盖，从而实现不同环境进行不同的部署；helm进行了更彻底的模块化，把资源清单打包成chart，定义了chart打包、发布到仓库和chart之间的依赖的解决方案。

更高级一些的方案是gitops，流行的方式包括flux和argocd。gitops的范式下，需要运维工作者提供一个gitops仓库，在这个仓库中提供要部署的资源清单，flux和argocd会持续监听该仓库的更新并应用到k8s中，这样做就可以对“部署和发布”这件事有更好的版本控制和回撤。
